...
\fntext{tf-2a}{%
  ...
  triangle (instead of ``\h{maj7}'' or ``\h{M7}'') is intended to save space and
...
\fntext{tf-7}{%
  ...
  T it is possible to define a single-type transformation in the union set
...
\fntext{tft-3a}{%
  ...
  same triad). We can define \slideS so that it acts in a similar way, but
...
\fntext{tft-4}{%
  ...
  second movement of the Fauré string quartet, mm.~36--39. (I am thankful to
  Julian Hook for pointing this example out to me.)}
...
\fntext{tft-5}{%
  ...
  vi chord) instead of \h{FM7} in m.~11; Levine's \emph{Jazz Theory Book} gives
...
\fntext{tft-6}{%
  ...
  the space only as V\tsup{7} chords; we will return to this limitation
...
\fntext{tfe-2a}{%
  ...
  moves the root of a V\tsup{7} down a perfect fifth. In theory, however,
...
\fntext{tfe-5a}{%
  ...
  were drawn in a manner parallel with standard \tf space, a ii\tsup{7} chord
  would be directly below the I\tsup{7} chord with the same root. As is the case
...
\fntext{tfe-5b}{%
  In our discussion of the diatonic transposition operator in Section
  \ref{subsec:inttrans}, we defined $t_k$ to be the operation that transposes
  the root of a chord $k$ steps inside a particular diatonic collection.
  There, we were interested in a particular 7-element set of chords (with
  their attendant qualities); here, the root is still transposed in a
  particular diatonic collection, but the resulting chord is still a major
  seventh chord.}
...
\fntext{tfe-7}{%
  Path consistency is described in \cite[25--28]{hook:2007}.}
