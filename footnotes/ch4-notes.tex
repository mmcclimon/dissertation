...
\fntext{csa-5}{%
  ...
  octave) is played with his left
  hand and the manzello---a modified soprano saxophone---is played with his
  right. Normally the left hand is positioned at the top of the saxophone, so
  playing it in this manner limits his note choices somewhat (the left-hand
  palm keys are unreachable with the right hand). We will see a concrete
...
\fntext{csa-8}{%
  % xxx need MFT citation
  All of the networks here are what Lewin calls ``figural'' networks ();
  since the $R$ transformations do not form a semigroup, a ``formal'' network
  (showing all possible transformations) is not possible. Hall suggests
  in a footnote \parentext{\citetitle[213n16]{hall:2009}} that the logical
