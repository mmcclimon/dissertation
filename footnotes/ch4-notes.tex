...
\fntext{cst-2}{%
  % xxx need citation about scale theory generally
  Nor will we have need of the extensive literature on the abstract
  structure of scales in general. For an overview, see ??.}

\fntext{cst-3}{%
  Bishop's primary source is \cite{grafnettles:1997}; \cite{berklee:harmony}
  covers much of the same material but in a more modern way (neither book
  mentions Russell at all). The Berklee method systematizes much of Russell's
  method in a way suitable for teaching undergraduate jazz musicians. Every
  tone in a scale, for example, is either a chord tone, an avoid note, or a
  ``tension'': an upper extension that colors the basic
  sound (\emph{Berklee Book of Jazz Harmony}, xi).}
