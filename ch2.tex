...
\end{displaymath}}%
%
EC is of course equivalent to TF $\cdot$ \textsc{7th} $\cdot$
...
\begin{align*}
  ...
  \mathrm{TF}_{\mathrm{blues}}(X\,) &= Y\text{, where}\ X = (x_r, x_t, x_s)
                                      \in \Sdom
                                      \text{ and}\
                                      Y=(\,y_r, y_t, y_s)=(x_r, x_t, x_s-1)
                                      \in S_{\mathrm{IMm7}}
\end{align*}}%
%
Note that TF\tsub{blues} is equivalent to TF $\odot$ \textsc{7th},
and also to $T_5$ (when applied to a V\tsup{7} chord; TF\tsub{blues} is
...
% here, steve rings, tonal qualia, etc.
