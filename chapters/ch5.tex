...
\ref{et:ss-seq-first-half} gives the bridge from Rollins's fourth chorus;
...
\begin{figure}[tbp]
  ...
  \caption[A non-sequential bridge from Rollins's solo.]{%
    A non-sequential bridge from Rollins's solo (mm.~117--20, 2:15).}
  \label{et:ss-seq-first-half}
\end{figure}

Because the bridge consists of chromatically descending \tf{}s, the most
...
solo. In the first of these, from the third chorus, Rollins plays the same
quarter-note pattern in all four bars, highlighting the $T_{11}$s of the
progression itself.\fn{et-5} The second example, from the fifth chorus, is
...
\begin{figure}[tbp]
  \centerGraphic{eps/xxx.pdf}
  \caption[Two sequential bridges from Sonny Rollins's solo.]{%
    Two sequential bridges from Sonny Rollins's solo (mm.~85--88, 1:48 and
    mm.~149--52, 2:42).}
