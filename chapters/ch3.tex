...
that we as analysts must now contend with.\fn{mts-a}
...
As noted above, the minor-third substitution is not explained in terms of its
voice leading in the jazz pedagogical literature, since it does not contain
the tonic-defining \sd4--\sd7 tritone. There is precedent, however, for the
...
be easy to write off this chord as an upper-neighbor harmony to the more
...
during this discussion to avoid straying too far afield from this topic.
...
\begin{figure}[p]
  \centerGraphic[width=0.65\textwidth]{eps/ch3/gs-cube-dance.pdf}
...
% \begin{figure}[p]
  \centerGraphic[width=0.55\textwidth]{eps/ch3/four-cube-trio.pdf}
...
\ref{maj3:four-cube-trio}.\fn{maj3-10} As is readily apparent, Four-Cube Trio
...
The arrangement into $T_8$-related ``layers'' also helps to clarify the
function of Coltrane changes as a substitution for a \tfo progression
...
\begin{tabular}{lcl}
  If $X = (x_r, x_t, x_s) \in \Smaj$, & then &
    $\mathrm{CS}(X\,) = Y = (\,y_r, y_t, y_s) = (x_r + 3, x_t + 3, x_s + 2)
                                               \in \Sdom$
\end{tabular}
\addtolength{\tabcolsep}{3pt}
\vspace{0.5\baselineskip}

\noindent With this transformation, we can understand the entire \tfo
...
of M3-space allows us to better understand the bridge. In ordinary \tf space
