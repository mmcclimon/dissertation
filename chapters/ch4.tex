...
concept of a \emph{scale index}, as shown in Table \ref{cst:scale-index}.
...
parent scale: scale 2 remains ``Lydian augmented'' rather than ``melodic
...
both elements are part of a mathematical group. Though this will not be the
final form of our chord-scale \gis, we will define this one more formally
...
or flats to a key signature, respectively.\fn{cst-10}
...
all the benefits of a \gis. We can understand a passage like the one in
Figure \ref{cst:gis-fiveone} as the \gis interval \scalepair{$s_1$}{$0$}:
where the collection changes but the scale does not. Figure
\ref{cst:gis-blues-lyd} shows a passage which begins with the F blues scale
and resolves to the F Lydian collection, representing the \gis interval
\scalepair{$e$}{$-7$}, where $e$ indicates the identity element.\fn{cst-13}
...
\subsection{Refining the GIS}
\label{subsec:refining-gis}
...
listeners have different experiences when hearing ``A4
as \sd1'' and ``A4 as \sd7.''\footcite[42]{rings:2011} In many ways, this
distinction is at the heart of what Russell was trying to accomplish in the
\emph{Lydian Chromatic Concept}. For Russell, the F acoustic collection played
over a tonic major-minor seventh chord really \emph{is} a different entity
than the same collection played over a dominant major-minor seventh chord.
This is what Russell means when he writes of ``a state of complete and
indestructible chord/scale unity'' (\lcc 10): if an \h{F7} chord can function
in more than one way, so too can its corresponding scale.

% Here: relax GIS a bit (from gis to quasi-gis), polar graph of chord-scales,
% adding chord symbol information to quasi-gis.
...
artificial, and the space becomes more intuitive if we simply use the integers
0--7. Of course, these integers do not form a group (they are not closed
under addition), and so the resulting space is not a proper \gis. It does
reflect the realities of Russell's system, though: the blues scale is the most
outgoing scale, and is maximally far away from the consonant Lydian. Intervals
in this new non-group come are taken from the set $\{-7, -6, \ldots , 6,
7\}$; except for the lack of closure, everything else about this almost-\gis
is identical.

We can visualize this space using the diagram in Figure \ref{cst:polar-gis}.
In this figure, the diatonic (Lydian) scale is centrally located, with more
outgoing scales located further toward the outside; these concentric circles
combine with the ordinary circle of fifths to divide each scale into diatonic
wedges.\fn{cst-17} The figure is inspired by Russell's description of the
Lydian as a ``musical `Star-Sun'\,'' (\lcc 8) and the ultimate source of tonal
gravity. More outgoing scales have more gravitational potential energy, as it
were, and are more dissonant with the underlying diatonic
collection. We can use this figure to map the two presentations of the F
acoustic scale of Figure \ref{cst:ambiguous-acoustic}; such a mapping is given
in Figure \ref{cst:polar-acoustic}. This visualization makes clear that the
second presentation (a tonic \h{F7} chord) is more outgoing than the first (a
dominant \h{F7} chord), as well as the shift in underlying diatonic collection
(\Eflat\ Lydian vs.\ F Lydian).
