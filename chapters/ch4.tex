...
\flat\sd7 appears before \nat\sd7\,).\fn{lcc-8a} The Lydian scale is also unique in that
...
system of chord-scales.\fn{lcc-14}
...
What Davis was looking for is essentially a way of determining
...
from Russell's theory outlined above.\fn{cst-3} He is also interested in
...
operations, and indeed these four scales account for six of Russell's seven
...
whether \sd2 and \nat\sd7 are included.\fn{cst-4a} While we could incorporate this scale into
...
while \scalepair{2\flat}{Lyd.\ \flat{}7} describes the \Eflat\ acoustic
collection (\Eflat Lydian is in the 2\flat diatonic collection, and the Lydian
\flat{}7 scale on \Eflat adds the pitch \Dflat).\fn{cst-8a} Creating an ordered-pair \gis
...
signature, respectively; we might write \mbox{1\sharp $\xrightarrow{\ s_1\ }$
  ...
  $\xrightarrow{\ s_3\ }$ 2\sharp}.\fn{cst-10}
...
transformations have the form $R$\scalepair{\emph{signature
    transformation}}{\emph{scale index interval}}. The transformation acts on
...
as \scalepair{\nat}{Lyd.}, rather than, say, \scalepair{2\sharp}{Lyd.\
  \flat{}7}; the \sd4 that would confirm either is absent. The problem seems
...
of music, and thus listeners have different experiences when hearing ``A\tsub{4} as
\sd1'' and ``A\tsub{4} as \sd7.''\footcite[42]{rings:2011} In many ways, this
...
spaces must be both symmetrical and homogeneous.\fn{cst-18} A \gis cannot account for
...
scale.\fn{cst-19} Nevertheless, the transformations still seem
...
timbre, intonation, and so on---are absent from the notation.\fn{csa-2}
...
particulars of Parker's or Coltrane's style.\fn{csa-3}
...
side.\fn{csa-3a} Two examples of enclosures appear in the \tfo progression in
...
diatonic collection to the whole--half diminished scale.\fn{csa-7}
...
taken from the last four bars of a chorus of ``Blues for Alice.'' \fn{csa-10}
...
chord-scale analysis of a passage.\fn{csa-12} The most common kind of metrical
...
is played as \scalepair{3\sharp}{Dia.}\ in this recording, while
\h{Bbm7}--\h{Eb7} is played as \scalepair{4\flat}{Dia.}---\,a difference of
five sharps/seven flats.\fn{csa-17a}
...
\begin{figure}[tbp]
  ...
  \caption[Two interpolated \ii chords from Henderson's first chorus.]{Two
    interpolated ii\tsup{7} chords from Henderson's first chorus (mm.~7--9, 0:39).}
...
Tymoczko's voice-leading spaces.\fn{csa-25}
...
the jazz experience'' (\lcc 223). This chapter has asked the related question
