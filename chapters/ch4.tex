...
% xxx clarify the distinction between GIS intervals and transformations here
...
0--7 (which we will call $S$ for the time being). The resulting space, though,
runs afoul of the formal requirements for a \gis, which requires that \ivls
form a mathematical group. The set $S$ under addition does not form a group,
since it is not closed under addition ($6+5$ is not a member of $S$\,), is not
associative ($1+1$ and $6+1$ are defined, but $[1 + 6] + 1$ is not), and
...
to be reasonable intuitions about the nature of chord-scales. As Hook notes,
``the narrative portions of Lewin's analyses generally far transcend the
logical consequences of the group structure,'' so the fact that a mathematical
group does not underlie this no-longer-\gis should not dissuade us
from exploring its analytical potential.\footcite[185]{hook:2007gmit}

Lewin does allow for transformations to form semigroups, but the scale index
...
are not possible from every scale. For example:
\mbox{int$\big($\scalepair{\nat}{Lyd.}, \scalepair{\nat}{Whole-tone}$\big)=$
  \scalepair{$e$}{$4$}}, and \mbox{int$\big($\scalepair{\nat}{Whole-tone},
  \scalepair{\nat}{Lyd.}$\big)=$ \scalepair{$e$}{$-4$}}, but there is no
chord-scale which satisfies $x$ in the equation
\mbox{int$\big($\scalepair{\nat}{Blues}, $x\big)=$ \scalepair{$e$}{$4$}},
since there is no scale that is four levels more outgoing than the blues
scale.
