...
in F.”\fn{tf-2} Given the importance of this progression for improvising jazz
...
\begin{figure}[tbp]
  ...
  transformation network for a \tfo\ in C major: \caph{Dm7}--\caph{G7}--\caph{Cmaj7}.}
...
here.\fn{tf-3} A transformation network consists of objects of some kind
...
itself.\fn{tf-4} The transformations in Figure \ref{ds:autumn-leaves-bridge}
...
\begin{figure}[tbp]
  \centerGraphic[width=.55\textwidth]{eps/ch2/mobius-dominants.pdf}
...
\end{tabular}
\addtolength{\tabcolsep}{3pt}
\vspace{0pt}    % don't know why this fixes weird spacing, but it does
...
\begin{figure}[tbp]
  ...
  \protect\h{Dm7} in mm.~5--6 of ``Ceora.'']{The
    \textsc{slide}\textsubscript{7} transformation from \protect\caph{Dbmaj7}
    to \protect\caph{Dm7} in mm.~5--6 of ``Ceora.''}
...
% Fmaj7 --> Em7 is very close to the voice leading of the bridge (if you infer a
% Bb7 chord, then you get Fmaj7 ---> Bb7 ---(ECt)---> Em7
...
\begin{figure}[tbp]
  \centerGraphic[width=.8\textwidth]{eps/ch2/tf-minor-tonics.pdf}
...
\noindent Note that TF\tsub{blues} is equivalent to $T_5$, but is defined in a
way to demonstrate its similarity to TF (see the voice leading in Figure
\ref{tfe:tf-blues-voice-leading}; TF\tsub{blues} is undefined on \ii chords).
We have also defined the set $S$\tsub{IMm7}, the set of tonic major-minor
seventh chords; this seems intuitive, but is somewhat complicated.
$S$\tsub{IMm7} is exactly equivalent to \Sdom---in the language of set theory,
they are the same set. The difference between them is not structural, but
interpretive: $S$\tsub{IMm7} is the set of \emph{tonic} major-minor seventh
...
\begin{figure}[tbp]
  \centerGraphic[width=.7\textwidth]{eps/ch2/blues-tf-space.pdf}
...
$\xrightarrow{7\text{\textsc{th}}\ \bullet\ T_1}$ \h{Fs7}.\fn{tfe-5}
...
and minor seventh chords sharing a root are adjacent in the space.\fn{tfe-5a}
...
of the 4-flat diatonic collection.\fn{tfe-5b} This $t_6$ operation affects
...
The arrangement of \tf space in Figure \ref{tfe:ab-diatonic-space}, combining
...
$t_6$.\fn{tfe-7} Put another way, putting \h{Gmaj7} in the top row of Figure
