...
\fntext{ds-6}{%
  ...
  attitude,” see \cite[24--29]{rings:2011}; \cite[102--3 and
  116--17]{satyendra:2004}; and \cite[172--77]{hook:2007gmit}.}
...
\fntext{ds-9}{%
  The diatonic transposition operator $t_k$ is described in
  \cite[139--40]{hook:2008}.}
...
\fntext{ds-11b}{%
  The symbol $\odot$ here represents function composition from left-to-right:
  $t_4 \odot t_3$ is read as ``$t_4$ followed by $t_3$.'' Mathematicians
  often use right-to-left orthography: $g(f\,(x))$ indicates ``apply $f\,$ to $x$,
  then apply $g$ to its result''; this is often indicated $g \circ f$. (Lewin uses
  left-to-right notation in \emph{GMIT}; see p.~2.) In this dissertation we
  will often be considering longer chains of transformations where right-to-left
  orthography would quickly become tedious, so we will use the symbol $\odot$
  exclusively to mean function composition from left to right. This usage
  follows that of Julian Hook, see \headlesscite[Section 2.3]{hook:mst}.}
...
\fntext{ds-18b}{%
  I tend to hear the opening of ``All the Things You Are'' in F minor, though
  it is certainly possible to hear it in \Aflat\ major instead. In either
  case, the diatonic \gis is nearly identical; the only difference is the
  quality of the C chord (\h{Cm7} in \Aflat, and \h{C7} in F minor). Readers
  who prefer to hear the opening in \Aflat\ can easily make the necessary
...
\fntext{ds-22}{%
  ...
  analysis here, that all of the phrase-ending major-sevenths in the first
  sixteen bars (\Aflat, C, \Eflat, G) spell out the tonic major-seventh chord;
