...
\section{Diatonic Chord Space}
\label{sec:diatonic-space} %{{{
...
sense, it also forms what David Lewin has called a Generalized Interval System
(GIS).\footcite[26]{lewin:gmit} Since this is the first of several GISs we
will encounter, it will be useful here to explore the concept in some depth.

% xxx a paragraph here on GISs and how they’re useful, etc.
[[[ to be written ]]]

A GIS ($S$, IVLS, int) consists of a space $S$, an interval group IVLS, and an
interval function of the space. Here, the space of the GIS is the set of
chords in the diatonic cycle: \mbox{$S =$ \{\h{GmM7},} \h{Am7b5}, \h{BfM7}, \h{Cm7},
\h{D7}, \h{EbM7}, \h{F7}\}. The interval group, IVLS, is the cyclic group
$\mathcal{C}_7 = \{t_0,t_1,\ldots,t_6\}$, where $t_k$ is the diatonic
transposition operator that transposes the root of a chord $k$ steps inside a
particular diatonic collection. The interval function maps $S \times S$ into
IVLS; the interval between two chords in $S$ is a member of the group
$\mathcal{C}_7$.
