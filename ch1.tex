...
the connections from chord to chord are all $t_3$ operations, continuing the
chain that has been present since the beginning (see Figure
...
third and fourth bars of the final A section,
...
connection to the bridge, then, can be understood as the inverse of the two
descending fifths that ended the A section. This observation can be
represented mathematically ($t_3^{-1} + t_3^{-1} = t_4 + t_4 = t_1$) or
...
easily understood as a combination of two $t_3$ operations, imagining that
...
two \mbox{ii\tsup{7}--V\tsup{7}} progressions, the first in F and the second
in \Eflat\ (which again does not appear in the music, like at the end of the
bridge). These \tf\ progressions are best stood in chromatic, rather than
diatonic, space; for now, we will pass over this progression until we have
developed such a space.
...
\h{CM7}, \h{Dm7}, \h{E7}, \h{FM7}, \h{G7}\}); the groups IVLS and the
...
keys, this chord is easily understood as a tonic.
...
move to the relative major is common, and the only difference between the two
diatonic sets is the shift of \h{E7} (in A minor) to \h{Em7} (in C major). The
progression here is not as systematic as the first eight (see the annotations
in Figure \ref{ds:wonderland-transformations}). After a C-major \tf\ in
mm.~9--10, the harmony moves up a step to \h{Em7}--\h{Am7}.\fn{ds-15} The
progression \h{G7}--\h{Em7} seems to move backwards in the cycle, almost as if
realizing the phrase is in danger of arriving at C major too soon. We can
capture this intution by choosing to understand the progression not as a
...
but rather to A minor. At some point, then, the progression shifts from being
a C-major cycle (the E chord is a minor seventh in the opening bar) to an
A-minor cycle. Unlike the abrupt shift to the relative major in “Alice in
Wonderland” (signalled by \h{Eb7}), this modulation is gradual, using a
...
% todo:
%   - All the Things You Are, heard diatonically
...
through the lens of diatonic seventh chord space reveals connections that may
...
a diatonic cycle (a chain of $t_3$ operations in an F-minor diatonic seventh
chord GIS) until arriving on \h{DbM7} in m.~5. At this point, the chord roots
continue to descend by diatonic fifth in the key of F minor, but the qualities
of the chords rooted on G and C have been altered, from \h{Gm7b5} and \h{C7}
(as they would be in the F-minor GIS) to \h{G7} and \h{CM7}. This arrival on a
C major chord (rather than a C dominant seventh) has the effect of a half
cadence in the prevailing key of F minor. The half-cadential C-major chord
also serves as a linking chord to the next phrase, which contains a diatonic
cycle in the key of C minor. Like the first A section, this phrase veers off
to end in a half cadence on G major.
...
true, we might understand this progression as an alteration of a diatonic
...
combination of two $t_3$ operations, interpolating the missing \h{CM7}
...
get \h{Gb7}--\h{Cm7}--\h{Bo7}. This progression is clearly not diatonic, so we
cannot say much about it at this point. It is interesting to note, however,
that harmony four bars from the end is a \h{Bbm7}, which is exactly where the
chain of $t_3$s would have ended, had it continued (see Figure
\ref{ds:all-things-last-a}). After this phrase expansion, the piece closes
with a \tfo\ in the key of \Aflat.

\figBeg[htbp]
  \centerGraphic{eps/xxx.pdf}
  \caption[Two versions of “All the Things You Are,” final twelve bars.]{The
  final twelve bars of “All the Things You Are.” Letter \emph{a} gives the
  changes as written, while letter \emph{b} gives a hypothetical version that
  continues the $t_3$ cycle in \mbox{F minor/}\Aflat\ major.}
...
of F minor, the succession of chord qualities at the beginning does not make
too much sense: two minor chords, followed by a dominant seventh, then two
major chords, all seemingly unrelated to the phrase-concluding tonic C
major.\fn{ds-21} Understanding the first 8 bars as diatonic, ending with a
...
attention is drawn to the connections between the spaces (key areas)
themselves, rather than the (comparatively boring) series of descending
diatonic fifths that occur within them.  While transformational analyses are
often accused of privileging chord-to-chord connections to the detriment of
long-range hearing, in this case it is the GIS framework developed above that
allows us to hear over longer distances where chord-to-chord connections may
fail to do so.

% xxx more here on All the Things?
