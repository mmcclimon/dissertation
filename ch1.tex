...
\subsection{Generalized Intervals and Transformations}
\label{subsec:gistrans} % {{{{
...
we might instead like to measure distance by the number of fifths. In this new
scheme, the interval from \h{Am7b5} to \h{D7} is not 3 (diatonic steps), but 1
(descending fifth).\fn{ds-5} Though this generation has no noticable effect on
the abstract group structure of IVLS (the group is $\mathcal{C}_7$ in either
case), it does affect the notion of distance in the space: see Figure
\ref{ds:c7-generators}.
...
% xxx to do here
% * converting GIS thinking to transformational thinking (intervals become t_k)
%   (Rings sec 1.3.3)
