...
It will be easiest to describe the transformational approach to jazz harmony
by way of an example. Much of this dissertation will be interested in the
development of various musical spaces.\fn{ds-1} These spaces can then be used
as a kind of shortcut to explore transformational relationships among a given
set of harmonies.
...
sense, it also depicts what David Lewin has called a Generalized Interval
System (GIS).\footcite[26]{lewin:gmit} Since this is the first of several GISs
we will encounter, it will be useful here to explore the concept in some
depth.  Generalized Interval Systems are Lewin’s way of formalizing the
...
intervals (IVLS). Though we could perhaps define a number of different ways to
define intervals among elements of the set S, the most obvious is to measure
distances between the chord roots. Because we are interested in the diatonic
system, we can use the diatonic transposition operator, $t_k$, as a measure of
these distances.\fn{ds-3} Because we are interested in abstract chord roots
and not the actual pitches played by some bass player or pianist’s left
hand, we will use diatonic pitch \emph{classes}. This has the effect of
modularizing the set of possible transpositions, changing IVLS from
$\{\ldots,t_{-2},t_{-1},t_{0},t_{1},t_{2},\ldots\}$ (as it would be in
diatonic \emph{pitch} space) to $\{t_0,t_1,\ldots,t_6\}$. Arithmetic in this
group is mod-7, exactly in the way that arithmetic using the more familiar
chromatic $T_k$ operator is mod-12.

Lewin specifies that IVLS must be a mathematical group, and we will take care
to show that IVLS = $\{t_0,t_1,\ldots,t_6\}$ is indeed such a group. A group
...
To show that IVLS is a group, it is sufficient to show that the set
$\{t_0,t_1,\ldots,t_6\}$ under some binary operation satisfies the group
axioms. The binary operation is addition of subscripts, performed mod-7 (which
we will denote $\oplus$ instead of $\otimes$ to avoid the implication of
multiplication); to determine the result of $t_1 \oplus t_3$, simply add the
subscripts to get $t_4$. We can then show that the set IVLS is closed: for any
two elements $a, b \in \mathrm{IVLS}$, $a \oplus b$ is also an element of IVLS
($t_1 \oplus t_3 = t_4; t_4 \oplus t_4 = t_1; t_1 \oplus t_0 = t_1$; and so
on). Because modular addition is associative, we know that $\oplus$ is
associative as well: $(t_1 \oplus t_2) \oplus t_3 = t_1 \oplus (t_2 \oplus
t_3)$, and likewise for any chosen elements of IVLS. The identity element of
the group is $t_0$, which combined with any element $a \in \mathrm{IVLS}$ gives $a$
itself. Inverses in the group are simply complements mod-7: $t_2^{-1} = t_5;
t_1^{-1} = t_6$; and so on. IVLS is isomorphic to the group $G =
(\mathbb{Z}_7, +)$, and it also a cyclic group generated by the element $t_1$,
so we will label it $\mathcal{C}_7$.
...
member of the group $\mathcal{C}_7$, discussed at length above. In our “Autumn
Leaves” example, the operator $t_k$ transposes the root of a chord $k$ steps
inside the G-minor diatonic collection, so the interval from one element of S
to another is that $t_k$ operator that produces the desired transposition.
Thus, \mbox{int(\h{D7}, \h{GmM7} = $t_3$}, since transposing the root, D,
three steps inside the G-minor diatonic collection gives G. Likewise,
\mbox{int(\h{Am7b5}, \h{BbM7}) = $t_1$}; \mbox{int(\h{F7}, \h{EbM7}) = $t_6$};
and so on.

% xxx did jay say something about this in the proposal?
...
our “Autumn Leaves” example, \mbox{int(Cm7,D7)int(D7,\h{EbM7})} must equal
\mbox{int(Cm7,\h{EbM7})}: $t_1t_1 = t_2$. Second, for every chord $s$ in S and
every interval $i$ in $\mathcal{C}_7$, there must be a unique chord $t$ which
satisfies the equation \mbox{int$(s,t) = i$}. For example, there must be
exactly one chord that lies $t_2$ units from \h{Cm7}: namely, \h{EbM7}.

% xxx need a bit about converting GIS thinking to transformational thinking
% (Rings sec 1.3.3)

% xxx expand this conclusion
Understood in this way, the chord progression of the A section of “Autumn
Leaves,” and consequently the circle of fifths arrangement in Figure
\ref{ds:circle-of-fifths}, can be understood as a series of $t_3$ transformations
within this G-minor diatonic seventh-chord space. In mod-12 chromatic space,
it can be difficult to make sense of this harmonic progression. The chord
qualities can be confusing (two major sevenths in a row followed immediately
by a half-diminished seventh), and it is sometimes hard to remember where the
tritone in the bass falls in the progression, given the prevalence of
perfect-fifth bass motion in the chromatic space. Understood in the context of
this diatonic space, however, the harmonic motion becomes much clearer.
